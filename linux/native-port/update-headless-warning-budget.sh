...
if [[ "${ALLOW_INCREASE}" != "1" ]]; then
  ...
  if [[ -n "${old_unique_warning_lines}" ]] && (( new_unique_warning_lines > old_unique_warning_lines )); then
    echo "[warning-budget-update] Refusing increase: unique warning lines ${old_unique_warning_lines} -> ${new_unique_warning_lines}"
    echo "[warning-budget-update] Re-run with ALLOW_INCREASE=1 to override"
    exit 1
  fi
  if [[ -n "${old_unique_actionable_lines}" ]] && (( new_unique_actionable_lines > old_unique_actionable_lines )); then
    echo "[warning-budget-update] Refusing increase: unique actionable lines ${old_unique_actionable_lines} -> ${new_unique_actionable_lines}"
    echo "[warning-budget-update] Re-run with ALLOW_INCREASE=1 to override"
    exit 1
  fi
fi
