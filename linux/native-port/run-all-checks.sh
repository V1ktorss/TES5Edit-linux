...
RUN_FPC_FALLBACK_AUDIT="${RUN_FPC_FALLBACK_AUDIT:-1}"
RUN_FPC_FALLBACK_BUDGET_GUARD="${RUN_FPC_FALLBACK_BUDGET_GUARD:-1}"
...
XEDIT_MODE_SANITY_TEST="${XEDIT_MODE_SANITY_TEST:-1}"
XEDIT_MODE_SANITY_ARGS="${XEDIT_MODE_SANITY_ARGS:--TES5 -dummy}"
...
if [[ "${RUN_FPC_FALLBACK_AUDIT}" == "1" ]]; then
  echo "[checks] FPC fallback surface audit"
  linux/native-port/audit-fpc-fallback-surface.sh
else
  echo "[checks] Skipping FPC fallback surface audit (RUN_FPC_FALLBACK_AUDIT=${RUN_FPC_FALLBACK_AUDIT})"
fi

if [[ "${RUN_FPC_FALLBACK_BUDGET_GUARD}" == "1" ]]; then
  echo "[checks] FPC fallback budget guard"
  linux/native-port/check-fpc-fallback-budget.sh
else
  echo "[checks] Skipping FPC fallback budget guard (RUN_FPC_FALLBACK_BUDGET_GUARD=${RUN_FPC_FALLBACK_BUDGET_GUARD})"
fi
...
if [[ "${RUN_XEDIT_HEADLESS}" == "1" ]]; then
  ...
  XEDIT_MODE_SANITY_TEST="${XEDIT_MODE_SANITY_TEST}" \
  XEDIT_MODE_SANITY_ARGS="${XEDIT_MODE_SANITY_ARGS}" \
