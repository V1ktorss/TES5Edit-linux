...
- Implemented `xeMainForm` message-surface audit script/report:
  ...
  - current baseline: 0 `message WM_*` bindings, 1 `TMessage` usage, 0 raw `WM_*` references
...
- xEdit smoke normal cases now also fail on unresolved mode markers (`Can't determine GameMode/ToolMode`) for earlier startup-regression detection
- xEdit env/cli override smoke lanes now also fail on unresolved mode markers for consistent startup-regression coverage across all lane types
...
- xDump base/env/cli smoke lanes now fail hard on mode-detection or unexpected-exception markers instead of logging non-fatal notes
...
- started `xeMainForm` service extraction for non-visual workflows:
  ...
  - moved Pluggy/GameLink folder path assembly into service helpers (`xeGetPluggyUserFilesFolder`, `xeGetGameLinkFolder`)
