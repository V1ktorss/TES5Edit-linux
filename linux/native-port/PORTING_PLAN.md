...
- FPC headless init no longer crashes in early FO4 common definitions:
  ...
  - reduced bootstrap currently advances through common setup up to `wbRegionAreas`
...
- FPC/Linux still requires reduced definition bootstrap in headless mode (currently exits after `wbRegionAreas`); full `DefineCommon` parity is not complete yet.
