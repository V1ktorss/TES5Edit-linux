...
- Started `xeMainForm` decoupling:
  ...
  - moved additional `pmuNavPopup` visibility gates into `xeMainFormServices` (`xeCanShowCleanupInjected`, `xeCanShowCreateSeqFile`, `xeCanShowLocalizationSwitch`) and switched form logic to explicit `IwbFile` resolution state (`IsFileElement`, `FileLoadOrder`)
...
- started `xeMainForm` service extraction for non-visual workflows:
  ...
  - moved existing-target rename planning (`has-existing`, backup filename policy, action text, timestamp warning) into service helper (`xeBuildExistingRenameTargetPlan`)
  ...
  - moved full per-file rename execution path (`prepare source`, `existing target handling`, `rename action text`, `final rename`) into service helper (`xeTryRunModuleRenameFlow`) and reduced `DoRenameModule` to UI/progress handling
  ...
  - moved final close-state teardown (`Files := nil`, `_wbProgressCallback := nil`, `ExitCode := CheckResult`) into service helper (`xeFinalizeMainFormCloseState`)
